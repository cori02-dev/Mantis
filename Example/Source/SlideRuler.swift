...
fileprivate let dotWidth: CGFloat = 3
...
class SlideRuler: UIView {
    ...
    public func setUIFrames() {
        ...
        pointer.frame = CGRect(x: (frame.width / 2 - pointerWidth / 2), y: bounds.origin.y+6, width: pointerWidth, height: frame.height-6)
        
        centralDot.frame = CGRect(x: frame.width - dotWidth / 2, y: 0, width: dotWidth, height: dotWidth)
        centralDot.path = UIBezierPath(ovalIn: centralDot.bounds).cgPath
        
        scaleBarLayer.frame = CGRect(x: frame.width / 2, y: 6, width: frame.width, height: frame.height-6)
    ...
    private func makePointer() {
        let pointerColor = UIColor(red: 255.0/255.0, green: 92.0/255.0, blue: 92.0/255.0, alpha: 1)
        pointer.backgroundColor = pointerColor.cgColor
        layer.addSublayer(pointer)
    }
    
    private func makeCentralDot() {
        let dotColor = UIColor(red: 255.0/255.0, green: 92.0/255.0, blue: 92.0/255.0, alpha: 1)
        centralDot.fillColor = dotColor.cgColor
        slider.layer.addSublayer(centralDot)
    }
    
    private func makeRuler() {
        scaleBarLayer.sublayers?.forEach { $0.removeFromSuperlayer() }
        let scaleColor = UIColor(red: 141.0/255.0, green: 141.0/255.0, blue: 151.0/255.0, alpha: 1)
        let scaleBar = makeBarScaleMark(byColor: scaleColor.cgColor)
        ...
        let majorColor = UIColor(red: 85.0/255.0, green: 85.0/255.0, blue: 90.0/255.0, alpha: 1)
        let majorScaleBar = makeBarScaleMark(byColor: majorColor.cgColor)
